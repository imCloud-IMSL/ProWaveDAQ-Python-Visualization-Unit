...
**功能**：更新即時資料緩衝區（供前端顯示），使用固定大小的循環緩衝區（NumPy Array）

**核心架構**：
- 緩衝區大小：`7812 Hz × 3 通道 × 10 秒 = 234,360 個資料點`
- 時間緩衝區大小：`10 個時間點`（對應 10 秒）
- 資料結構：使用 NumPy Array（`np.ndarray`），初始化為 0
- 寫入策略：資料從第10秒位置開始寫入，前9秒保持為0
...
1. **檢查活躍連線**
   ...
   - 使用 `data_request_lock` 確保執行緒安全

2. **條件更新緩衝區**
   - **如果有活躍連線且已開始收集**：
     - 取得資料鎖定（`data_lock`）
     - 將輸入資料轉換為 NumPy Array（`dtype=np.float64`）
     - 計算當前時間（相對於 `collection_start_time` 的秒數）
     - 計算第10秒的起始位置：`tenth_second_start = one_second_data_points * 9`
     - 如果首次寫入（`realtime_data_count == 0`），將寫入索引設為第10秒位置
   
3. **資料寫入邏輯**
   - 將資料寫入循環緩衝區的當前位置（`realtime_write_index`）
   - 更新時間緩衝區：只更新最後一個元素（索引9），存儲當前時間
   - 如果時間小於9秒，時間值設為0.0
   
4. **循環推進邏輯**（當資料超過10秒時）
   - 當寫入索引會循環到第1秒位置時：
     - 將第10秒的資料移動到第1秒
     - 將第1秒到第9秒的資料往前移動（第1秒→第2秒，第2秒→第3秒...，第9秒→第10秒）
     - 清空第1秒位置（因為資料已經移動到第2秒了）
     - 將寫入索引重置為第10秒位置，繼續寫入新資料
     - 同步更新時間陣列：將時間往前移動
   - 確保資料持續往前推進，不會停留在同一位置

5. **更新計數器**
   - 更新 `realtime_data_count`（已寫入的資料點數）
   - 更新 `data_counter`（總資料點數，用於狀態顯示）

**設計理由**：
- 固定大小緩衝區：避免記憶體無限增長
- 前9秒為0：確保資料從圖表右側（第10秒位置）開始顯示
- 循環推進：當資料超過10秒時，自動將舊資料往前移動，保持顯示連續性
- 智慧更新：僅在有活躍前端連線時更新，節省 CPU 和記憶體資源
- CSV 寫入不受影響，確保資料不遺失
...
**運作流程**：
1. 取得資料鎖定（`data_lock`）
2. 計算1秒的資料量：`one_second_data_points = current_sample_rate * 3`
3. 計算第10秒的起始位置：`tenth_second_start = one_second_data_points * 9`
4. 始終返回完整的緩衝區（不重新排列）
   - 資料陣列：前9秒為0，第10秒有資料（從 `tenth_second_start` 開始）
   - 時間陣列：前9個元素為0，最後一個元素有時間值
5. 返回資料的副本（避免前端修改原始資料）

**設計理由**：
- 不重新排列資料：確保資料不會跳到前面，始終保持前9秒為0，第10秒有最新資料
- 返回完整緩衝區：前端可以根據需要處理資料
- 執行緒安全：使用鎖定確保資料一致性
