...
### 技術特性
- **降頻佇列架構**：使用 `web_data_queue` 存儲降頻後的資料（50:1 降頻），大幅減少前端資料傳輸量和繪圖負擔
- **增量更新機制**：前端使用增量更新方式，只處理新資料，維持固定視窗大小（500 點）
...
- **即時資料可視化**：使用 Chart.js 實現即時圖表（每 100ms 更新），關閉動畫以提升效能
...
- **降頻架構**：降頻佇列（Downsampling Queue）
  ...
  - 前端繪圖負擔大幅降低
- **佇列架構**：
  - `web_data_queue`：存儲降頻後的資料供前端使用（最大 10,000 筆）
  - `csv_data_queue`：存儲原始資料供 CSV 寫入（最大 1,000 筆）
  - `sql_data_queue`：存儲原始資料供 SQL 上傳（最大 1,000 筆）
- **顯示限制**：前端顯示最多 500 個點（約 5-10 秒的降頻資料）
- **增量更新**：前端只處理新資料，使用 `push()` 和 `splice()` 維持固定視窗大小
