...
### 技術特性
- **固定大小循環緩衝區**：使用 NumPy Array 建立固定大小的循環緩衝區（234,360 個資料點），避免記憶體無限增長
- **資料推進機制**：資料從第10秒位置開始寫入，當資料超過10秒時自動往前推進，確保顯示連續性
- **智慧緩衝區更新**：僅在有活躍前端連線時更新緩衝區，節省 CPU 和記憶體資源
- **多執行緒架構**：5 個獨立執行緒（Flask、DAQ Reading、Collection、CSV Writer、SQL Writer），確保各組件不互相干擾
- **執行緒安全通訊**：使用 `queue.Queue` 進行執行緒間通訊，確保資料一致性
- **即時資料可視化**：使用 Chart.js 實現即時圖表（每 200ms 更新），顯示最近 10 秒的資料
...
- **資料傳輸**：JSON 格式
- **緩衝區架構**：固定大小循環緩衝區（NumPy Array）
  - 資料緩衝區：固定 234,360 個資料點（7812 Hz × 3 通道 × 10 秒）
  - 時間緩衝區：固定 10 個時間點（對應 10 秒）
  - 資料從第10秒位置開始寫入，前9秒保持為0
  - 當資料超過10秒時，自動將舊資料往前推進（第10秒→第1秒→第2秒...→第9秒→第10秒）
- **顯示限制**：前端顯示最近 10 秒的資料（每個通道約 78,120 個資料點）
- **優化機制**：智慧緩衝區更新（僅在有活躍連線時更新，5 秒內有請求視為活躍）
